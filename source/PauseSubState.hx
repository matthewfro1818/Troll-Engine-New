...
class PauseSubState extends MusicBeatSubstate
{
	...
	override public function create()
	{
		...
		for (daText in songInfo){
			...
			prevText.setFormat(Paths.font(PlayState.instance.gameFont), 32, 0xFFFFFFFF, RIGHT);
		...
		if (PlayState.chartingMode){
			...
			chartingText.setFormat(Paths.font(PlayState.instance.gameFont), 32);
	...
	function regenMenu():Void {
		...
		for (i in 0...menuItems.length) {
			...
			if(menuItems[i] == 'Skip Time')
			{
				...
				skipTimeText.setFormat(Paths.font(PlayState.instance.gameFont), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
