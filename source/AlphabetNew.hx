...
class AlphabetNew extends FlxSpriteGroup
{
	public var text(default, set):String;

	public var bold:Bool = false;
	public var letters:Array<AlphaCharacterNew> = [];
	...
	public function clearLetters()
	{
		...
		{
			--i;
			var letter:AlphaCharacterNew = letters[i];
			if(letter != null)
			{
				letter.kill();
	...
	private function createLetters(newText:String)
	{
		...
		for (character in newText.split(''))
		{
			...
			if(character != '\n')
			{
				...
				var isAlphabet:Bool = AlphaCharacterNew.isTypeAlphabet(character.toLowerCase());
				if (AlphaCharacterNew.allLetters.exists(character.toLowerCase()) && (!bold || !spaceChar))
				{
					...
					var letter:AlphaCharacterNew = new AlphaCharacterNew(xPos, rows * Y_PER_ROW * scaleY, character, bold, this);
...
class AlphaCharacterNew extends FlxSprite
{
	...
	public function new(x:Float, y:Float, character:String, bold:Bool, parent:AlphabetNew)
	{
		super(x, y);
		this.parent = parent;
		image = 'alphabetNew';
