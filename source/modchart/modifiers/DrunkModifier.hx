...
class DrunkModifier extends NoteModifier {
	...
	override function getPos( visualDiff:Float, timeDiff:Float, beat:Float, pos:Vector3, data:Int, player:Int, obj:FlxSprite, field:NoteField)
	{
		...
		// direction-specific
		pos.x += applyDrunk('$data', player, time, visualDiff, data) + applyTipsy('X$data', player, time, visualDiff, data)
			+ applyBumpy('X$data', player, time, visualDiff, data);
		pos.y += applyDrunk('Y$data', player, time, visualDiff, data) + applyTipsy('$data', player, time, visualDiff, data)
			+ applyBumpy('Y$data', player, time, visualDiff, data);
		pos.z += applyDrunk('Z$data', player, time, visualDiff, data) + applyTipsy('Z$data', player, time, visualDiff, data)
    ...
    override function getSubmods(){
		var axes = ["X", "Y", "Z"];
		var props = [["Speed", "Offset", "Period"], ["Speed", "Offset"], ["Offset", "Period"]];
		...
		for (i in 0...shids.length)
		{
			...
			for (a in 0...axes.length)
			{
				...
				var p = props[i];
				for (prop in p)
					submods.push('$mod$axe$prop');

				for (d in 0...4)
				{
					submods.push('$mod$axe$d');
					for (prop in p)
						submods.push('$mod$axe$d$prop');
				}
			}
		}
