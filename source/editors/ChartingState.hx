...
class ChartingState extends MusicBeatState
{
	...
	var hudList:Array<String> = ['Default'];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var skinDropdown = new FlxUIDropDownMenuCustom(player1DropDown.x, player2DropDown.y + 40, FlxUIDropDownMenuCustom.makeStrIdLabelArray(skins, true),
		...
		noteSplashesInputText = new FlxUIInputText(player2DropDown.x, player2DropDown.y + 50, 150, _song.splashSkin, 8);
	...
	var noteSkinInputTextbf:FlxUIDropDownMenuCustom;
	var noteSkinInputTextdad:FlxUIDropDownMenuCustom;
	...
	function addNoteUI():Void
	{
		...
		var key2:Int = 0;
		var displayNameList2:Array<String> = [];
		while (key2 < noteStyleList.length) {
		...
		{
			_song.arrowSkinbf = displayNameList2[Std.parseInt(character)];
		});
		noteSkinInputTextbf.selectedLabel = _song.arrowSkinbf;
		blockPressWhileScrolling.push(noteSkinInputTextbf);

		noteSkinInputTextdad = new FlxUIDropDownMenuCustom(noteTypeDropDown.x + 150, noteTypeDropDown.y + 45, FlxUIDropDownMenuCustom.makeStrIdLabelArray(displayNameList2, true), function(character:String)
		{
			_song.arrowSkindad = displayNameList2[Std.parseInt(character)];
		});
		noteSkinInputTextdad.selectedLabel = _song.arrowSkindad;
		blockPressWhileScrolling.push(noteSkinInputTextdad);

		tab_group_note.add(new FlxText(10, 10, 0, 'Sustain length:'));
		tab_group_note.add(new FlxText(10, 50, 0, 'Strum time (in miliseconds):'));
		tab_group_note.add(new FlxText(10, 90, 0, 'Note type:'));
		tab_group_note.add(new FlxText(noteSkinInputTextbf.x, noteSkinInputTextbf.y - 15, 0, 'Bf Note Texture:'));
		tab_group_note.add(new FlxText(noteSkinInputTextdad.x, noteSkinInputTextdad.y - 15, 0, 'Dad Note Texture:'));
	...
	function reloadGridLayer() 
	{
		...
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, leHeight, true, gridColor1, gridColor2);

		if (nextStartTime <= FlxG.sound.music.length && nextBeats > 0)
		{
			var nextHeight:Int = Std.int(GRID_SIZE * nextBeats * 4 * zoomList[curZoom]);

			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, nextHeight,true, gridColor1, gridColor2);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("Normal Text.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			...
			if(t >= startThing && t < endThing)
			{
				...
				daText.setFormat(Paths.font("Normal Text.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
